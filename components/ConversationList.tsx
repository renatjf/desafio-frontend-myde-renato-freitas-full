    ...
    <div className="flex flex-col h-full overflow-hidden bg-[var(--sidebar-bg)]">
      <div className="flex-shrink-0 bg-[var(--accent)] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-semibold text-sm tracking-wide">Myde Atendimento</span>
        </div>
        <div className="flex items-center gap-2">
          {totalUnread > 0 && (
          ...
          <span title="Sincronizando" className="text-[10px] text-white opacity-70">●</span>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 px-3 py-2 bg-[var(--sidebar-bg)] border-b border-[var(--border)]">
        ...
        <div className="mt-2">
          ...
          >
            Não lidas {filterUnread && totalUnread > 0 ? `(${totalUnread})` : ""}
          </button>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto min-h-0" role="list" aria-label="Lista de conversas">
        {isLoading && (
        ...
        {isError && (
          <ErrorState message="Não foi possível carregar as conversas." onRetry={() => refetch()} />
        )}
          ...
          <EmptyState
            ...
            title={
              search ? "Nenhuma conversa encontrada"
                : filterUnread ? "Sem mensagens não lidas"
                : "Nenhuma conversa ainda"
            }
            description={
              search ? "Tente outro nome ou mensagem."
                : filterUnread ? "Todas as mensagens foram lidas."
        ...
        {!isLoading && !isError && filtered.map((conversation) => (
          <div role="listitem" key={conversation.id}>
            <ConversationItem conversation={conversation} searchQuery={search} />
          </div>
        ))}
      </div>
    </div>
